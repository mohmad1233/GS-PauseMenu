...
CreateThread(function()
    ...
    RequestStreamedTextureDict('gs_pause', true)
    ...
    while not HasStreamedTextureDictLoaded("gs_pause") do
        Wait(100)
    end
    while true do
        if IsPauseMenuActive() then
            SetScriptGfxDrawBehindPausemenu(true)
            DrawSprite("gs_pause", Config.Background, 0.5, 0.5, 1.0, 1.0, 0, 255, 255, 255, Config.Opacity)
            ...
            SetStreamedTextureDictAsNoLongerNeeded('gs_pause')
            Wait(150)
        end
    end
end)
